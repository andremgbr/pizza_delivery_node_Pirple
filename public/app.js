            ...
            allForms[i].addEventListener("submit", function (e) {
                ...
                if (formId == "placeOrder") {
                    ...
                    for (var i = 0; i < Object.keys(payload).length; i++) {
                        if (Object.values(payload)[i] == true) {
                            list.push(Object.keys(payload)[i])
                        }
                    ...
                    payload = {
                        'idArray': list.toString().replace("[", "").replace("]", ""),
                        'cardNumber': payload['cardNumber'],
                        'cardExp_month': payload['cardExp_month'],
                        'cardExp_year': payload['cardExp_year'],
...
app.formResponseProcessor = function (formId, requestPayload, responsePayload) {
    ...
    if (formId == 'placeOrder') {
        ...
        window.location.href = responsePayload['urlReceipt'];
    }
    ...
    if (phone && tokenId) {
        ...
        app.client.request(undefined, 'api/menu', 'GET', queryStringObject, undefined, function (statusCode, responsePayload) {
            ...
            if (statusCode == 200) {
                ...
                var allChecks = typeof (responsePayload) == 'object'  ? responsePayload : [];
                if (Object.keys(allChecks).length > 0) {
                    Object.keys(allChecks).forEach(function (checkId) {

                        var newQueryStringObject = {
                            'id': checkId,
                            'phone':phone
                        };
                        app.client.request(undefined, 'api/itens', 'GET', newQueryStringObject, undefined, function (statusCode, responsePayload) {
                            if (statusCode == 200) {

                                var table = document.getElementById("checksListTable");
                                var tr = table.insertRow(-1);
                                tr.classList.add(responsePayload.id);
                                ...
                                box.setAttribute("name",responsePayload.id);
                                td0.appendChild(box);
                                   
                                td1.innerHTML = responsePayload.name;
                                td2.innerHTML = responsePayload.price;
                                td3.innerHTML = responsePayload.ingredientes;
...
window.onload = function () {
    app.init();
    console.log("iniciou o app.init");
};;